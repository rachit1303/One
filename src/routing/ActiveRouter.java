...
public abstract class ActiveRouter extends MessageRouter {
	...
	protected int startTransfer_2(Message m, Connection con) {
		int retVal;
		...
		retVal = con.startTransfer_2(getHost(), m);
		if (retVal == RCV_OK) { // started transfer
			addToSendingConnections(con);
		} else if (deleteDelivered && retVal == DENIED_OLD
				&& m.getTo() == con.getOtherNode(this.getHost())) {
	...
	protected boolean makeRoomForMessage(int size) {
		...
		// Always return true if it is a PBS --> PBS has unlimited buffer
...
*/
	...
	protected Message tryMessages(Connection con, Message m) {
		int retVal = startTransfer(m, con);
		if (retVal == RCV_OK) {
			return m; // accepted a message, don't try others
		} else if (retVal > 0) {
	...
	// new
	protected Message tryAllMessages_withsameD(Connection con,
		...
		for (Message m : messages) {
			...
			int ToNodeAddress = ToNode.getAddress();
			int FromNodeAddress = FromNode.getAddress();
			...
			if (ToNodeName.contains("Boat") && FromNodeName.contains("PBS")) {
				...
				int CurrentPBSIndex = PBS_Sequence.indexOf(FromNodeName);

				// If message is on PBS
				if (this.getHost().compareTo(ToNode) != 0) {

					String[] temp = BoatDP.get(ToNodeAddress);
					int CurrentDPIndex;
					for (int i = 0; i != temp.length; i++) {
						CurrentDPIndex = PBS_Sequence.indexOf(temp[i]);
						if (CurrentDPIndex < CurrentPBSIndex) {

							int retVal = startTransfer_2(m, con);
							if (retVal == RCV_OK) {
								return m; // accepted a message, don't try);
											// others
							} else if (retVal > 0) {
								return null; // should try later -> don't bother
												// trying others
							}
						}
					}
				...
				// If message is on Boat
					...
					for (int k = 0; k != temp.length; k++) {
						currentIndex = PBS_Sequence.indexOf(temp[k]);
						if (currentIndex < SmallestIndex) {
							SmallestIndex = currentIndex;
						}
					}

					if (SmallestIndex >= CurrentPBSIndex) {
						int retVal = startTransfer_2(m, con);
						if (retVal == RCV_OK) {
							return m; // accepted a message, don't try); others
						} else if (retVal > 0) {
			...
			// If One node is PBS and the other node is in Boat
			else if (ToNodeName.contains("PBS")
					&& FromNodeName.contains("Boat")) {

				int CurrentPBSIndex = PBS_Sequence.indexOf(ToNodeName);

				// If message is in PBS
				if (this.getHost().compareTo(FromNode) != 0) {

					String[] temp = BoatDP.get(FromNodeAddress);
			...
			else if (ToNodeName.contains("Boat")
					&& FromNodeName.contains("Boat")) {
				...
				int SmallestIndex1 = 99;
				int currentIndex1;
				String[] temp1 = BoatDP.get(FromNodeAddress);
				for (int k = 0; k != temp1.length; k++) {
					currentIndex1 = PBS_Sequence.indexOf(temp1[k]);
					if (currentIndex1 < SmallestIndex1) {
						SmallestIndex1 = currentIndex1;
					}
				}
	...
	protected Connection tryMessageToConnection(List<Connection> connections,
			Message m, List<Integer> Path) {
		...
		for (int i = 0, n = connections.size(); i < n; i++) {
			...
			if (Path.contains(ToNodeAddress) && Path.contains(FromNodeAddress)) {
				Message started = tryMessages(con, m);
			}
		}
		return null;
	}
	...
	// New
	protected Connection tryMessagesToConnections_withsameD(
			List<Message> messages, List<Connection> connections,
			ArrayList<String[]> BoatDP, ArrayList<String> PBS_Sequence) {
		for (int i = 0, n = connections.size(); i < n; i++) {
			Connection con = connections.get(i);
			Message started = tryAllMessages_withsameD(con, messages, BoatDP,
					PBS_Sequence);
			if (started != null) {
				return con;
			}
	...
	// /New
	protected Connection tryAllMessagesToAllConnections_withsameD(
			ArrayList<String[]> BoatDP, ArrayList<String> PBS_Sequence) {
		List<Connection> connections = getConnections();
		...
		return tryMessagesToConnections_withsameD(messages, connections,
				BoatDP, PBS_Sequence);
	}
